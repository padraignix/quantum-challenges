...
def oracle_matrix(indices):
    ...
    ### Take indices and make it so indices with 1 flip an estimation wire??
...
def circuit(indices):
    ...
    ### Initial Hadamards
    qml.Hadamard(target_wires)
    qml.Hadamard(estimation_wires)

    ### Have to apply Grover matrix
    ### application_matrix = grover_operator(indices)
    ### "apply" application_matrix to qml node
...
def relative_error(indices):
    ...
    ### calling number of solutions to get "sol"
    ### sol = number_of_solutions(indices)
    ### real_sol = how many items in indices
    ### rel_err = (sol - real_sol)/real_sol * 100%
    rel_err = 
